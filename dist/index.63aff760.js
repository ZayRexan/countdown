...
function dateSplit() {
    ...
    recived = (0, _momentDefault.default)(dateStr);
}
function calculations() {
    let now = (0, _momentDefault.default)();
    rest = {
        years: recived.diff(now, "years"),
        days: recived.diff(now, "days"),
        hours: recived.diff(now, "hours")
    };
    if (rest.days >= 365) rest.days = Math.round(rest.days % 365);
    if (rest.hours >= 24) rest.hours = Math.round(rest.hours % 24);
}
function render() {
    if (rest.years <= 0 && rest.days <= 0 && rest.hours <= 0) alert("Прошлое уже не вернуть...");
    if (rest.years > 0) output.year.innerText = `${rest.years} лет`;
    else output.year.innerText = "";
    if (rest.days > 0) output.day.innerText = `${rest.days} дней`;
    else output.day.innerText = "";
    if (rest.hours > 0) output.hour.innerText = `${rest.hours} часов`;
    else output.hour.innerText = "";
}
...
},{}]},["9ZIyz","adjPd"], "adjPd", "parcelRequire4482")
